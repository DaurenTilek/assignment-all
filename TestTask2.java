...
class pericomp implements Comparator<PlanarShape>{
    @Override
...
class distcomp implements Comparator<PlanarShape>{
    @Override
    public int compare(PlanarShape s1,PlanarShape s2){
...
public class TestTask2 {
    public static void main(String[] args ){
        ...
        PlanarShape maxP=Collections.max(list,new pericomp());
        System.out.println(maxP.getPerimiter());

        PlanarShape minP=Collections.min(list,new pericomp());
